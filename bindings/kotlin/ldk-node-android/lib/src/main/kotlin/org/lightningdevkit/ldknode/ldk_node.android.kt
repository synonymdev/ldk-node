...
internal interface UniffiLib : Library {
    ...
    fun uniffi_ldk_node_fn_method_node_add_onchain_wallet_account_with_mnemonic(
        `ptr`: Pointer?,
        `account`: RustBufferByValue,
        `mnemonic`: RustBufferByValue,
        `passphrase`: RustBufferByValue,
    ...
    fun uniffi_ldk_node_checksum_method_node_add_onchain_wallet_account_with_mnemonic(
    ): Short
...
private fun uniffiCheckApiChecksums(lib: UniffiLib) {
    ...
    if (lib.uniffi_ldk_node_checksum_method_node_add_onchain_wallet_account() != 8042.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    if (lib.uniffi_ldk_node_checksum_method_node_add_onchain_wallet_account_with_mnemonic() != 62673.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    ...
    if (lib.uniffi_ldk_node_checksum_method_node_get_balance_for_onchain_wallet_account() != 14472.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
...
open class Node: Disposable, NodeInterface {
    ...
    @Throws(NodeException::class)
    override fun `addOnchainWalletAccount`(`account`: OnchainWalletAccount, `seedBytes`: List<kotlin.UByte>): kotlin.String {
        return FfiConverterString.lift(callWithPointer {
            uniffiRustCallWithError(NodeExceptionErrorHandler) { uniffiRustCallStatus ->
    ...
    @Throws(NodeException::class)
    override fun `addOnchainWalletAccountWithMnemonic`(`account`: OnchainWalletAccount, `mnemonic`: Mnemonic, `passphrase`: kotlin.String?): kotlin.String {
        return FfiConverterString.lift(callWithPointer {
            uniffiRustCallWithError(NodeExceptionErrorHandler) { uniffiRustCallStatus ->
                UniffiLib.INSTANCE.uniffi_ldk_node_fn_method_node_add_onchain_wallet_account_with_mnemonic(
                    it,
                    FfiConverterTypeOnchainWalletAccount.lower(`account`),
                    FfiConverterTypeMnemonic.lower(`mnemonic`),
                    FfiConverterOptionalString.lower(`passphrase`),
    ...
    @Throws(NodeException::class)
    override fun `getBalanceForOnchainWalletAccount`(`account`: OnchainWalletAccount): AddressTypeBalance {
        return FfiConverterTypeAddressTypeBalance.lift(callWithPointer {
            uniffiRustCallWithError(NodeExceptionErrorHandler) { uniffiRustCallStatus ->
...
object FfiConverterSequenceTypeOnchainWalletAccount: FfiConverterRustBuffer<List<OnchainWalletAccount>> {
    override fun read(buf: ByteBuffer): List<OnchainWalletAccount> {
        ...
        return List<OnchainWalletAccount>(len) {
            FfiConverterTypeOnchainWalletAccount.read(buf)
        }
    }

    override fun allocationSize(value: List<OnchainWalletAccount>): ULong {
        val sizeForLength = 4UL
        val sizeForItems = value.sumOf { FfiConverterTypeOnchainWalletAccount.allocationSize(it) }
        return sizeForLength + sizeForItems
    }

    override fun write(value: List<OnchainWalletAccount>, buf: ByteBuffer) {
        buf.putInt(value.size)
        value.iterator().forEach {
