	...
	($node:expr, $payment_id:expr, $payment_hash:expr, $claimable_amount_msat:expr) => {{
		loop {
			match $node.next_event_async().await {
				...
				ref e if $crate::common::is_informational_event(e) => {
					println!("{} skipping event {:?}", std::stringify!($node), e);
					$node.event_handled().unwrap();
					continue;
				},
				ref e => {
...
pub(crate) async fn do_channel_full_cycle<E: ElectrumApi>(
	...
	// Don't drain here - we need to expect ChannelReady events below
	// The macros will skip informational events automatically
