...
macro_rules! impl_read_write_change_set_type {
	(
		...
		where
			L::Target: LdkLogger,
		{
			let address_type_suffix = match address_type {
				crate::config::AddressType::Legacy => "legacy",
				crate::config::AddressType::NestedSegwit => "nested_segwit",
				crate::config::AddressType::NativeSegwit => "native_segwit",
				crate::config::AddressType::Taproot => "taproot",
			};
			let secondary_namespace = if $secondary_namespace.is_empty() {
				address_type_suffix.to_string()
			} else {
				format!("{}/{}", $secondary_namespace, address_type_suffix)
			};
			let bytes =
